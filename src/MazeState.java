...
/**
 * Reads the maze stored in an intermediate array into a more useful structure
 * that stores the state of the maze and its associated components.
 * 
 * @author dcyoung
 *
 */
public class MazeState {
	...
	public void readInitialState(String filename) {
		...
		for (int row = 0; row < numRows; row++) {
			for (int col = 0; col < numCols; col++) {
				...
				switch (arrayVal) {
				case 1:// if grid space is a wall
	...
	/**
	 * Populate the neighborNodes ArrayList for each node with any adjacent
	 * nodes that are not walls
	 */
	private void updateNeighborNodes() {
		System.out.println("\t MS: Updating Neighbor Nodes");

		// if neighboring nodes are only allowed to be up/down or right/left
		// (ie: no diagonals)
				...
				for (int row = r - 1; row <= r + 1; row++) {
					for (int col = c - 1; col <= c + 1; col++) {
						if (row < 0 || col < 0 || row >= numRows || col >= numCols) {
							// do not add this neighbor... because the it is
							// outside the boundaries of the maze
						}
						// only consider the up/down and side/side movements,
						// not the diagonals or the current cell itself
						else if ((Math.abs(row - r) + Math.abs(col - c)) == 1) {
							// check if its a wall
							if (!state[row][col].isWall()) {
								// if the neighboring node is not a wall, add it
		...
		/*
		 * System.out.println(
		 * "\t NeighboringNodes: Diagonal movement considered."); for(int r = 0;
		 * r < numRows; r++){ for(int c = 0; c < numCols; c++){ for(int row =
		 * r-1; row <= r+1; row++ ){ for(int col = c-1; col <= c+1; col++ ){ if(
		 * (row == r && col == c) || row < 0 || col < 0 || row >= numRows || col
		 * >= numCols ){ //do not add this neighbor... because }else{
		 * if(!state[row][col].isWall() ){ //if the neighboring node is not a
		 * wall, add it to the neighbor nodes
		 * state[r][c].getNeighborNodes().add(state[row][col]); } }
		 * 
		 * } } } }
		 */
	...
	public static void main(String[] args) {
		 ...
		 * Arrays.asList("bigMaze.txt", "mediumMaze.txt", "openMaze.txt",
		 * "tricky1.txt", "bigGhost.txt");
		 * 
		 * System.out.println("Creating maze state"); MazeState mazeState = new
		 * MazeState(filenames.get(4));
		 * 
		 * DrawingBoard dB = new DrawingBoard(mazeState);
		 * dB.drawMazeInitialState();
		 * dB.drawVisit(mazeState.ghostStartNode.getRow(),
		 * mazeState.ghostStartNode.getCol(), 200, StdDraw.RED); TreeNode ghost
		 * = mazeState.ghostStartNode; TreeNode oldGhost = ghost; for(int i = 1;
		 * i <100; i++){
		 * 
		 * ghost = mazeState.DetermineGhostNode(i);
		 * dB.drawVisit(oldGhost.getRow(), oldGhost.getCol(), 200,
		 * StdDraw.WHITE); dB.drawVisit(ghost.getRow(), ghost.getCol(), 200,
		 * StdDraw.RED); oldGhost = ghost; }
		 */
	}
