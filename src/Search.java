...
/**
 * Search class: A collection of search algorithms to solve a maze provided a
 * mazestate. This class was mainly for prototyping has not yet been refactored.
 * It is fully operational, but contains various code smells.
 * 
 * @author dcyoung
 *
 */
public class Search {
	 ...
	 * 
	 * @return the goal state node once found, null if never found Can read the
	 *         path used to find the goal by using node.getParent()
	 */
	...
	/**
	 * Performs greedy search on a maze state
	 * 
	 * @return
	 */
			...
			if (neighbors != null && neighbors.size() != 0) {
				...
				for (TreeNode n : neighbors) {
					...
					// or the new hcost to neighbor is shorter
						...
						// remove the neighbor from open set in order to change
						// its fcost and reinsert it in sorted order
						open.remove(n);
						// set the fcost of n
						n.setgCost(current.getgCost() + 1);
						n.sethCost(CalculateDistance(n, ms.getGoalNode(), false));
						// set parent
						n.setParent(current);
						// reinsert the neighbor into the open set in the
						// correct order with its new fcost
						open.add(n);
					}
				}
			}
	...
	/**
	 * Performs A* search on a maze state
	 * 
	 * @return
	 */
	 ...
	 * @param n2
	 * @param useEuclidian
	 *            -> true if euclidian distance is desired, false if manhattan
	 * @return
	 */
	...
	private float CalculateCustomHeuristic(TreeNode n1, TreeNode n2, int turnCost) {
		...
		// return (float)( turnCost +
		// wallDensity*(n1.getgCost()+CalculateDistance(n1,n2, false)) );
	}
		...
		while (!open.isEmpty()) {
			...
			if (neighbors != null && neighbors.size() != 0) {
				...
				for (TreeNode n : neighbors) {
					...
					// or the new path to neighbor is shorter
					else if (n.getgCost() > (current.getgCost() + CalculateMoveCost(FindOrientation(n, current),
							FindOrientation(current, current.getParent()), forwardCost, rotateCost))) {
						// remove the neighbor from open set in order to change
						// its fcost and reinsert
						// it in sorted order
						open.remove(n);
						// set the fcost of n
						n.setgCost(current.getgCost() + CalculateMoveCost(FindOrientation(n, current),
								FindOrientation(current, current.getParent()), forwardCost, rotateCost));
						if (bUseManhattanDist)
							n.sethCost(CalculateDistance(n, ms.getGoalNode(), false));
						else
							n.sethCost(
									CalculateCustomHeuristic(n, ms.getGoalNode(),
											CalculateMoveCost(FindOrientation(n, current),
													FindOrientation(current, current.getParent()), forwardCost,
													rotateCost)));
						// set parent
						n.setParent(current);
						// reinsert the neighbor into the open set in the
	...
	/**
	 * Visit a node, set the nodes visited flag, draw an indication on the
	 * drawing board and make note of the new visit in the tally of expanded
	 * nodes.
	 * 
	 * @param n
	 */
	...
	/**
	 * Print the path of nodes that resulted from a search method starting First
	 * node printed is the starting loc, last node printed is the goal
	 * 
	 * @param path
	 */
	private static void PrintPath(ArrayList<TreeNode> path, boolean bPrintGCost, boolean bPrintHCost,
			boolean bPrintFCost) {
	...
	public static void main(String[] args) {
		List<String> filenames = Arrays.asList("bigMaze.txt", "mediumMaze.txt", "openMaze.txt", "tricky1.txt",
				"smallGhost.txt", "mediumGhost.txt", "bigGhost.txt", "smallTurns.txt", "bigTurns.txt");
